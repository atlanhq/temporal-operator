...
//
// A plain split on commas is enough because the only label values compared here
// are Kubernetes object names and the configured relation. A value that did
// contain a comma would fail to match, and an unmatched sample is reported as
// absent, which refuses.
